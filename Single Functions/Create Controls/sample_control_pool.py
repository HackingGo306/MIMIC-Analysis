...
def create_pool(pool_num = 0):
  ...
  fracture_patients = pd.read_csv(copyTitle + '/Control Group 4/' + category_name + ' subset.csv')
  ...
  patient_count = bank['subject_id'].nunique()

  subset_size = fracture_patient_count * 20
  ...
  while (added < subset_size):
    if (added % 1000 == 0):
      print(added, "of", subset_size)
    # choose a random row
    random_index = np.random.randint(0, len(bank))
    
    try:
      random_patient_id = bank.at[random_index, 'subject_id']
  ...
  # write
  fracture_patients.to_csv(copyTitle + '/Control Group 4/Unmatched/' + category_name + '/' + str(pool_num) + '.csv', index=False)
