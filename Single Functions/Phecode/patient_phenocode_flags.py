...
data_file = pd.read_csv(copyTitle + '/patient_data_phecode.csv')
...
for i in range(len(patient_diagnoses_file)):
  ...
  diagnosis_time = datetime.strptime(diagnosis_time, '%Y/%m/%d %H:%M:%S %p')
  diagnosis_category = patient_diagnoses_file['category'][i]
  
  if (pd.isnull(diagnosis_category)):
    continue
...
data_file.to_csv(copyTitle + '/patient_data_phecode_flags.csv', index=False)
